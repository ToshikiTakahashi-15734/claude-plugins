...
| **ステータスライン** | 画面下部に現在セッションのコンテキスト比(①)を常時表示（`👤 60% ███░ 🤖 40% [start]`） |

#### ステータスラインの2モード

`~/.claude/pr-contribution.mode` で切替（環境変数 `PR_CONTRIB_MODE_FILE` で変更可）。

| モード | 挙動 |
|---|---|
| `human-start`（既定） | AIが何も生成していない状態は**人間100%**から始まり、生成するほど実比率へ収束 `(H+K)/(H+A+K)` |
| `raw` | 生の累積比率 `H/(H+A)` |
